...
r17 = intake({
    ...
    "timeframe": "4h",
    "direction": "short",
    "thesis": "SOL breaking down with bear flag pattern; invalidated if price closes back above flag resistance.",
    "stop_loss": "3%",
    "max_loss_usdt": 300,
    "position_size_usdt": 2000,
    "risk_unit_usdt": 200,
...
r18 = intake({
    ...
    "timeframe": "4h",
    "direction": "long",
    "thesis": "BNB double bottom at range low with RSI divergence; invalidated if price closes below range low.",
    "stop_loss": "3%",
    "max_loss_usdt": 250,
    "position_size_usdt": 2000,
    "risk_unit_usdt": 200,
    ...
    if "execution_receipt_id" in r18_plan:
        ...
        if "outcome_id" in r18_outcome:
            r18_review = submit_review({
                ...
                "new_rule_candidate": None,
                "outcome_ref_type": "finance_manual_outcome",
                "outcome_ref_id": r18_outcome["outcome_id"],
            })
runs.append({
    "tag": "Run 18",
    "intake": r18,
...
r21 = intake({
    ...
    "timeframe": "1d",
    "direction": "long",
    "thesis": "INJ holding above 200 MA, not sure if this holds though; invalidated if price closes below 200 MA.",
    "stop_loss": "5%",
    "max_loss_usdt": 100,
    "position_size_usdt": 500,
    "risk_unit_usdt": 50,
    ...
    if "execution_receipt_id" in r22_plan:
        r22_outcome = outcome(
            r22["id"],
            {
                ...
                "verdict": "validated",
                "variance_summary": "Better than expected.",
                "plan_followed": True,
            },
        )
    ...
    if "execution_receipt_id" in r25_plan:
        r25_outcome = outcome(
            r25["id"],
            {
                ...
                "verdict": "validated",
                "variance_summary": "Pattern played out cleanly.",
                "plan_followed": True,
            },
        )
        if "outcome_id" in r25_outcome:
            r25_review = submit_review({
                ...
                "outcome_ref_type": "finance_manual_outcome",
                "outcome_ref_id": r25_outcome["outcome_id"],
            })
runs.append({
    "tag": "Run 25",
    "intake": r25,
    ...
    if "execution_receipt_id" in r28_plan:
        r28_outcome = outcome(
            r28["id"],
            {
                ...
                "verdict": "validated",
                "variance_summary": "Exceeded target.",
                "plan_followed": True,
            },
        )
        if "outcome_id" in r28_outcome:
            r28_review = submit_review({
                ...
                "deviation": "Exceeded target",
                "mistake_tags": "support_bounce",
                "lessons": [
                    {"lesson_text": "Bullish engulfing at weekly support with OBV divergence is a powerful signal."}
                ],
                "new_rule_candidate": None,
                "outcome_ref_type": "finance_manual_outcome",
                "outcome_ref_id": r28_outcome["outcome_id"],
            })
runs.append({
    "tag": "Run 28",
    "intake": r28,
...
for i, run in enumerate(runs, 1):
    ...
    print(
        f"{tag}: intake={intake_id} → gov={gov_dec} | plan={plan_id} | outcome={outcome_id} | review={review_id} (status={rev_completed})"
    )
    if reasons:
        for r in reasons:
            print(f"       reason: {r}")
