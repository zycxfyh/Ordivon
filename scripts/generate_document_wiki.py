...
def generate_wiki(entries: list[dict]) -> str:
    ...
    truth_docs = _filter(sorted_entries, authority=["source_of_truth", "current_status"])
    for e in truth_docs:
        lines.append(_row(e))
    lines.append("")
    ...
    ev_docs = [
        e for e in sorted_entries if e.get("doc_type") in ev_types or e.get("authority") == "supporting_evidence"
    ]
    seen = set()
    for e in ev_docs:
        did = e.get("doc_id")
